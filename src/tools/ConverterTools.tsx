  ...
  const addImages = (files: File[]) => {
    ...
    imageFiles.forEach(f => {
      ...
      reader.onload = (e) => {
        ...
        setImages(prev => [...prev, { file: f, url: dataUrl }]);
      };
      ...
      <Card className="border-0 shadow-lg shadow-black/5 bg-card/50 backdrop-blur-sm">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button onClick={generatePdf} disabled={loading || images.length === 0} className="flex-1 h-12 text-lg font-semibold rounded-xl gap-2 min-w-0">
              <Download className="h-5 w-5 flex-shrink-0" />
              <span className="truncate">{loading ? "Generating PDF…" : `Convert to PDF (${images.length} image${images.length !== 1 ? "s" : ""})`}</span>
            </Button>
            {images.length > 0 && (
              <Button variant="outline" onClick={() => setImages([])} className="h-12 rounded-xl px-4 flex-shrink-0">Clear All</Button>
            )}
          </div>
        </CardContent>
      </Card>
